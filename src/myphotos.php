...
<section>
    ...
    <div id="div-main">
        ...
        try {
            ...
            $sql = "select ImageID from travelimage where UID = '$strUID'";
            $result = $pdo->query($sql);
            $arrayImageID = []; $i = 0;
            while($row = $result->fetch()){
            ...
            for ($i = ($currentPage-1)*$pageSize; $i < min($k,(($currentPage-1)*$pageSize)+6); $i++){
                ...
                while($row = $result->fetch()) {
                    ...
                    echo "</span><br>
                <button class=\"modify\" onclick=window.location.href=\"uploadModify.php?id=".$vari;
                    echo "\">Modify</button>
                <button class=\"delete\" onclick=window.location.href=\"myphotosDelete.php?id=".$vari;
                    echo "\">Remove</button>
                    </div></div>";
                }
            }
            if ($i == 0){
                echo "<h1>您还没有上传照片，赶紧点击个人中心的上传按钮增加一张吧！</h1>";
            }
            if ($i != 0){
        ...
        /*<div class="div-favorites" id="div-favorites-bottom">
            <a href="details.php"><img src="../img/normal-medium/8710289254.jpg" alt="" class="img-favorites"></a>
            <div class="description">
                <h3>Title</h3>
                <span>This is a descriptive paragraph. I don't know what to put into this. Nor do I know how I managed to spend
                two hours and a half on such a simple page. I'm so tired after making this page (first page made after the
                homepage). Hope that ctrl+c & ctrl+v ing will make it much faster for other pages. I just want to go to bed
                    now zzzzzzzzzzz. I'm making this sentence longer than three lines so that some words can be omitted.</span><br>
                <a href="upload.php"><button class="modify">Modify</button></a>
